...
هر بار اجرا میشه (هر ۶ ساعت):
  ...
  2) داده کندل (OHLCV) از Binance برای هرکدوم میگیره و تحلیل تکنیکال میزنه
     (RSI, MACD, SMA20/50, ATR, حجم نسبت به میانگین)
  3) اخبار اخیر مرتبط با هر کوین رو از RSS های خبری چک می‌کنه
  4) احساسات اجتماعی (Bullish/Bearish) رو از StockTwits میگیره
  5) یه امتیاز نهایی (Score) و برای کوین‌های قوی، هدف (TP) و حد ضرر (SL) حساب می‌کنه
  6) نتیجه رو به صورت پیام تلگرام میفرسته و سیگنال‌های همین اجرا رو ذخیره می‌کنه
...
def get_technical_analysis(symbol):
    ...
    try:
        ...
        # ATR (میانگین محدوده نوسان واقعی) - برای محاسبه هدف/حد ضرر بر اساس نوسان واقعی کوین
        ...
        return {
            ...
            "atr": float(atr) if atr == atr else None,  # atr==atr چک NaN
        }
    except Exception:
        return None
...
def save_signals(strong_bullish, strong_bearish):
    ...
    for r in strong_bullish:
        ...
        if levels:
            signals.append({
                "symbol": r["symbol"], "name": r["name"], "direction": "long",
                "entry": levels["entry"], "tp": levels["tp"], "sl": levels["sl"],
                "score": r["score"], "generated_at": time.time(),
            })
    for r in strong_bearish:
        levels = calc_trade_levels(r.get("ta_data"), "short")
        if levels:
            signals.append({
                "symbol": r["symbol"], "name": r["name"], "direction": "short",
                "entry": levels["entry"], "tp": levels["tp"], "sl": levels["sl"],
                "score": r["score"], "generated_at": time.time(),
            })
...
def get_current_prices():
    """همه قیمت‌های لحظه‌ای بایننس رو تو یه درخواست می‌گیره (به‌جای یه درخواست به‌ازای هر کوین)"""
...
def build_coin_block(r, direction):
    """
    یه بلوک کامل برای یه کوین می‌سازه: امتیاز، قیمت، هدف/ضرر، و دلایل (هر کدوم خط جدا).
    direction: 'long' برای سیگنال صعودی، 'short' برای نزولی
    """
    emoji = "🟢" if direction == "long" else "🔴"
    lines = [f"{emoji} <b>{r['symbol']}</b> ({r['name']}) — امتیاز: <code>{r['score']}</code>"]
...
def build_report(results, performance_section=None):
    ...
    lines.append(f"🔍 تحلیل <code>{len(results)}</code> کوین برتر")
    lines.append("")
    ...
    if strong_bullish:
        lines.append("━━━━━━━━━━━━━━━━━━")
        lines.append("🟢 <b>سیگنال‌های صعودی قوی (پتانسیل خرید)</b>")
        lines.append("━━━━━━━━━━━━━━━━━━")
        for r in strong_bullish:
            lines.append(build_coin_block(r, "long"))
            lines.append("")

    if strong_bearish:
        lines.append("━━━━━━━━━━━━━━━━━━")
        lines.append("🔴 <b>سیگنال‌های نزولی قوی (پتانسیل فروش/هشدار)</b>")
        lines.append("━━━━━━━━━━━━━━━━━━")
        for r in strong_bearish:
            lines.append(build_coin_block(r, "short"))
            lines.append("")

    # خلاصه کل بازار داخل بلوک <pre> که تلگرام مجبوره چپ‌به‌راست و با فونت یکنواخت نشون بده
    # (این‌جوری دیگه فارسی و عدد قاطی هم نمی‌شن و به‌هم نمی‌ریزن)
    lines.append("📋 <b>خلاصه کل بازار (بر اساس امتیاز):</b>")
    summary_rows = "\n".join(f"{r['symbol']:<8} {r['score']:>5}" for r in results_sorted)
    lines.append(f"<pre>{summary_rows}</pre>")

    lines.append("⚠️ این تحلیل صرفاً جمع‌بندی داده‌هاست، مشاوره مالی نیست و TP/SL بر پایه نوسان اخیر (ATR) محاسبه شده — نه پیش‌بینی قطعی.")
    return "\n".join(lines)
...
def main():
    ...
    # ذخیره سیگنال‌های همین اجرا برای مقایسه تو اجرای بعدی (۶ ساعت دیگه)
    results_sorted = sorted(results, key=lambda x: x["score"], reverse=True)
    strong_bullish = [r for r in results_sorted if r["score"] >= 2][:10]
    strong_bearish = [r for r in results_sorted if r["score"] <= -1.5][:5]
    save_signals(strong_bullish, strong_bearish)
