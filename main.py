...
----------------------------------------
هر بار اجرا میشه (هر ۳ ساعت):
  0) عملکرد سیگنال‌های ۳ ساعت قبل رو چک می‌کنه - نه فقط قیمت لحظه‌ای الان، بلکه کل مسیر
     قیمت از لحظه صدور سیگنال تا الان رو می‌بینه (تا اگه قیمت رفته بالا به هدف خورده و
     برگشته، درست تشخیص بده)
  1) لیست ۸۰ کوین برتر از CoinGecko میگیره (استیبل‌کوین‌ها مثل USDT/USDC حذف می‌شن چون
     سیگنال معناداری تولید نمی‌کنن)
...
REQUEST_TIMEOUT = 15
REQUEST_DELAY = 2.2        # فاصله بین درخواست‌ها برای رعایت rate-limit رایگان CoinGecko
...
# لیست استیبل‌کوین‌های رایج - hardcode شده به‌جای گرفتن از API تا یه درخواست کمتر بزنیم
# (و اگه CoinGecko کند جواب داد، این بخش گیر نکنه)
STABLECOIN_SYMBOLS = {
    "USDT", "USDC", "DAI", "BUSD", "TUSD", "USDD", "FDUSD", "USDP", "GUSD",
    "USDE", "PYUSD", "USDS", "RLUSD", "USDG", "FRAX", "LUSD", "USDX", "EURT",
    "USDJ", "SUSD", "CUSD", "USTC", "USDK", "MIM", "USD1",
}


# ---------------- ابزار مشترک: درخواست به CoinGecko با مدیریت خودکار rate-limit (429) ----------------
def cg_get(url, params=None, max_retries=4):
    """
    مثل requests.get ولی اگه CoinGecko جواب 429 (خیلی درخواست) بده، به‌جای crash کردن
    صبر می‌کنه و دوباره امتحان می‌کنه. برای همه درخواست‌های CoinGecko تو این فایل استفاده میشه.
    """
    for attempt in range(max_retries):
        try:
            ...
            if r.status_code == 429:
                ...
                print(f"⏳ محدودیت نرخ CoinGecko (429) - {wait} ثانیه صبر می‌کنیم (تلاش {attempt + 1}/{max_retries})...")
                time.sleep(wait)
                continue
            r.raise_for_status()
            return r
            ...
            if attempt == max_retries - 1:
                raise
            wait = 5 * (attempt + 1)
            print(f"⚠️ خطای شبکه ({e}) - {wait} ثانیه صبر و تلاش دوباره...")
            time.sleep(wait)
    return None


# ---------------- گام ۱: گرفتن لیست کوین‌ها (بدون استیبل‌کوین‌ها) ----------------
def get_top_coins(n=TOP_N):
    url = "https://api.coingecko.com/api/v3/coins/markets"
    params = {
        "vs_currency": "usd",
        "order": "market_cap_desc",
        "per_page": 250,  # بیشتر از n می‌گیریم چون بعد از حذف استیبل‌کوین‌ها باید n تا واقعی باقی بمونه
        "page": 1,
        "sparkline": "false",
        "price_change_percentage": "24h",
    }
    r = cg_get(url, params=params)
    all_coins = r.json()
    filtered = [c for c in all_coins if c["symbol"].upper() not in STABLECOIN_SYMBOLS]
    return filtered[:n]
...
def get_current_prices(coin_ids):
    """قیمت لحظه‌ای چند کوین رو با CoinGecko تو یه درخواست می‌گیره (fallback وقتی تاریخچه در دسترس نیست)"""
    if not coin_ids:
        return {}
...
def get_price_history(coin_id, from_ts, to_ts):
    """
    تاریخچه قیمت یه کوین رو بین دو زمان مشخص می‌گیره (ریزتر از یه نقطه لحظه‌ای).
    برای بازه‌های کوتاه (مثل ۳ ساعت) CoinGecko خودش داده تقریباً هر ۵ دقیقه برمی‌گردونه.
    """
...
def evaluate_previous_signals(previous_signals):
    """
    برای هر سیگنال قبلی، کل مسیر قیمت رو از لحظه صدور سیگنال تا الان چک می‌کنه
    (نه فقط قیمت لحظه‌ای الان) تا ببینه اولین باری که قیمت به TP یا SL خورده کِی بوده.
    این‌جوری اگه قیمت رفته بالا به هدف خورده و بعد برگشته پایین، ربات درست تشخیص می‌ده.
    """
    evaluated = []
    now_ts = time.time()
    ...
    for sig in previous_signals:
        ...
        if not coin_id or not generated_at:
            continue

        entry, tp, sl = sig["entry"], sig["tp"], sig["sl"]
        direction = sig["direction"]

        price_series = get_price_history(coin_id, generated_at, now_ts)

        if not price_series:
            # اگه تاریخچه در دسترس نبود (مثلاً به‌خاطر rate-limit)، حداقل قیمت لحظه‌ای رو می‌گیریم
            fallback = get_current_prices([coin_id])
            current_price = fallback.get(coin_id)
            if current_price is None:
                continue
            price_series = [[now_ts * 1000, current_price]]
        ...
        if status == "tp_hit":
            ...
            pnl_pct = (sl - entry) / entry * 100 if direction == "long" else (entry - sl) / entry * 100
        else:
        ...
        time.sleep(REQUEST_DELAY)  # رعایت rate-limit - تعداد کم (فقط سیگنال‌های قبلی)
...
def build_performance_section(evaluated_signals):
    ...
    lines = []
    lines.append("━━━━━━━━━━━━━━━━━━")
    lines.append("📈 <b>بررسی عملکرد سیگنال‌های ۳ ساعت قبل</b>")
    lines.append("━━━━━━━━━━━━━━━━━━")
...
def build_report(results, performance_section=None):
    ...
    if not bullish and not bearish:
        lines.append("😐 این ۳ ساعت هیچ سیگنال قوی و قابل‌اتکایی تو ۸۰ کوین برتر پیدا نشد.")
        lines.append("")
...
def main():
    ...
    if previous_signals:
        ...
        evaluated = evaluate_previous_signals(previous_signals)
    ...
    # ذخیره فقط سیگنال‌های نمایش‌داده‌شده همین اجرا برای مقایسه تو اجرای بعدی (۳ ساعت دیگه)
    bullish, bearish = select_top_signals(results)
