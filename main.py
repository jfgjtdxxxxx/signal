...
def get_stablecoin_ids():
    """لیست شناسه استیبل‌کوین‌ها رو از دسته‌بندی خود CoinGecko می‌گیره تا از تحلیل کنار بذاریمشون"""
...
def get_top_coins(n=TOP_N):
    stablecoin_ids = get_stablecoin_ids()
    url = "https://api.coingecko.com/api/v3/coins/markets"
    params = {
        "vs_currency": "usd",
    ...
    r.raise_for_status()
    all_coins = r.json()
    filtered = [c for c in all_coins if c["id"] not in stablecoin_ids]
    return filtered[:n]
...
def get_price_history(coin_id, from_ts, to_ts):
    ...
    برای بازه‌های کوتاه (مثل ۶ ساعت) CoinGecko خودش داده تقریباً هر ۵ دقیقه برمی‌گردونه.
    """
